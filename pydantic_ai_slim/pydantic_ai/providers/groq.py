...
class GroqProvider(Provider[AsyncGroq]):
    ...
    def model_profile(self, model_name: str) -> ModelProfile | None:
        mn = model_name.lower()
        # The order of prefix matching is unchanged from original
        if mn.startswith('llama'):
            return meta_model_profile(model_name)
        if mn.startswith('meta-llama/'):
            # Strip off the matched prefix for subsequent logic
            return meta_model_profile(model_name[len('meta-llama/'):])
        if mn.startswith('gemma'):
        ...
        if mn.startswith('mistral'):
            return mistral_model_profile(model_name)
        if mn.startswith('moonshotai/'):
            # Strip off the matched prefix for subsequent logic
